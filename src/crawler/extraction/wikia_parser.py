...
class WikiaParser:
    ...
    def __init__(
        ...
        self.wikia_selectors = {
            ...
            "content_area": [
                ".page-content", ".mw-content-text", "#content", "main"
            ],
            "related_articles": [
                ".related-articles", ".see-also", ".references"
            ],
        }
        ...
        self.namespace_patterns = {
            ...
            "Organization": (
                r"^/wiki/(Organization:|Organizations:|Faction:|Factions:|)"
            ),
        }

    def should_crawl_page(
            self, url: str, soup: Optional[BeautifulSoup] = None
    ) -> bool:
        """Determine if page should be crawled based on URL and
        content."""
        if not url:
            return False
        ...
        if soup:
            # Skip disambiguation pages (low value for relationships)
            page_text = soup.get_text().lower()
            if ("disambiguation" in page_text or
                    "may refer to:" in page_text):
                return False

        return True

    def extract_wikia_content(
            self, soup: BeautifulSoup, url: str
    ) -> Dict:
        """Extract Wikia-specific structured content with simplified
        approach."""
        if not soup:
            return {}
    ...
    def extract_related_articles(self, soup: BeautifulSoup) -> List[str]:
        ...
        see_also_headings = soup.find_all(
            ["h2", "h3", "h4"],
            string=lambda text: text and "see also" in text.lower()
        )
    ...
    def _clean_wikia_navigation(
            self, soup: BeautifulSoup
    ) -> BeautifulSoup:
        """Remove Wikia navigation elements. WARNING: Modifies soup
        in-place."""
        if not soup:
            return soup
