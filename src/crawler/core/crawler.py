...
class WikiaCrawler:
    ...
    async def crawl_wikia(
        self, start_urls: List[str], max_pages: Optional[int] = None
    ) -> Dict:
        ...
        try:
            ...
            while True:
                ...
                if (
                    stats["pages_crawled"] % save_every == 0
                    and stats["pages_crawled"] > 0
                ):
    ...
    async def _should_crawl_url_async(self, url: str) -> bool:
        """Check if URL should be crawled based on filters and
        robots.txt (async version)."""
        if not self._is_valid_url(url):
            return False
