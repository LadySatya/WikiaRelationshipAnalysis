...
def log_phase_start(module: str, phase: str, details: Optional[Dict[str, Any]] = None):
...
def log_phase_end(module: str, phase: str, summary: Optional[Dict[str, Any]] = None):
    """Log the end of a processing phase."""
    logger = get_logger(module)
    summary_str = " | ".join(f"{k}={v}" for k, v in (summary or {}).items())
    logger.info(f"{'='*60}")
    logger.info(f"PHASE END: {phase}")
    if summary_str:
        logger.info(f"Summary: {summary_str}")
    logger.info(f"{'='*60}")


def log_progress(module: str, current: int, total: int, item: str = "items"):
    """Log progress for long-running operations."""
    logger = get_logger(module)
    percent = (current / total * 100) if total > 0 else 0
    logger.info(f"Progress: {current}/{total} {item} ({percent:.1f}%)")


# Legacy compatibility with old setup_project_logger function
def setup_project_logger(project_name: str, log_level: int = logging.INFO):
    """
    Legacy function for backward compatibility.
    Prefer using setup_logging() for new code.
    """
    level_name = logging.getLevelName(log_level)
    log_dir = setup_logging(project_name, log_level=level_name)
