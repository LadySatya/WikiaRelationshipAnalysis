...
class TestCrawlWikia:
    ...
    def mock_components(self):
        ...
        with (
            patch("src.crawler.core.crawler.RateLimiter") as mock_rate_limiter,
    ...
    async def test_crawl_wikia_invalid_urls(self, crawler_with_mocks):
        ...
        invalid_urls = ["not_a_url", "ftp://invalid.protocol.com", "https://", ""]
    ...
    @pytest.mark.asyncio
    async def test_crawl_wikia_single_url_success(
        self, crawler_with_mocks, mock_components
    ):
        ...
        crawler_with_mocks._crawl_page = AsyncMock(
            return_value={
        ...
        result = await crawler_with_mocks.crawl_wikia(
            ["https://naruto.fandom.com/wiki/Naruto_Uzumaki"], max_pages=1
        )
    ...
    @pytest.mark.asyncio
    async def test_crawl_wikia_multiple_start_urls(
        self, crawler_with_mocks, mock_components
    ):
        ...
        crawler_with_mocks._crawl_page = AsyncMock(
            return_value={"url": "test_url", "title": "Test", "content": "content"}
        )
    ...
    @pytest.mark.asyncio
    async def test_crawl_wikia_max_pages_enforcement(
        self, crawler_with_mocks, mock_components
    ):
        ...
        result = await crawler_with_mocks.crawl_wikia(
            ["https://naruto.fandom.com/start"], max_pages=3
        )
    ...
    @pytest.mark.asyncio
    async def test_crawl_wikia_calls_rate_limiter(
        self, crawler_with_mocks, mock_components
    ):
    ...
    @pytest.mark.asyncio
    async def test_crawl_wikia_handles_page_crawl_errors(
        self, crawler_with_mocks, mock_components
    ):
        ...
        result = await crawler_with_mocks.crawl_wikia(
            ["https://naruto.fandom.com/start"], max_pages=2
        )
    ...
    @pytest.mark.asyncio
    async def test_crawl_wikia_handles_url_manager_errors(
        self, crawler_with_mocks, mock_components
    ):
        """Test handling of URL manager errors."""
        mock_components["url_manager"].add_urls.side_effect = Exception(
            "URL manager error"
        )
    ...
    @pytest.mark.asyncio
    async def test_crawl_wikia_saves_state_periodically(
        self, crawler_with_mocks, mock_components
    ):
    ...
    # Return Value Tests
    @pytest.mark.asyncio
    async def test_crawl_wikia_returns_complete_statistics(
        self, crawler_with_mocks, mock_components
    ):
    ...
    @pytest.mark.asyncio
    async def test_crawl_wikia_no_max_pages_limit(
        self, crawler_with_mocks, mock_components
    ):
        ...
        mock_components["url_manager"].get_next_url.side_effect = available_urls + [
            None
        ]
