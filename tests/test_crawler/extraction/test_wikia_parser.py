...
class TestWikiaParserLinkProcessing:
    ...
    def test_is_same_wikia_domain_exact_match(self, wikia_parser):
        ...
        assert wikia_parser._is_same_wikia_domain(
            "https://buffy.fandom.com/wiki/Angel", base_url
        )
        assert wikia_parser._is_same_wikia_domain(
            "/wiki/Spike", base_url
        )  # Relative URL
        ...
        assert not wikia_parser._is_same_wikia_domain(
            "https://naruto.fandom.com/wiki/Naruto", base_url
        )
        assert not wikia_parser._is_same_wikia_domain(
            "https://witcher.fandom.com/wiki/Geralt", base_url
        )
    ...
    def test_is_same_wikia_domain_excludes_fandom_meta(self, wikia_parser):
        ...
        assert not wikia_parser._is_same_wikia_domain(
            "https://community.fandom.com/wiki/Central", base_url
        )
        assert not wikia_parser._is_same_wikia_domain(
            "https://about.fandom.com/careers", base_url
        )
        assert not wikia_parser._is_same_wikia_domain(
            "https://auth.fandom.com/signin", base_url
        )
        assert not wikia_parser._is_same_wikia_domain(
            "https://fandom.zendesk.com/help", base_url
        )
    ...
    def test_normalize_url_handles_different_formats(self, wikia_parser):
        ...
        # Relative URLs
        assert (
            wikia_parser._normalize_url("/wiki/Angel", base_url)
