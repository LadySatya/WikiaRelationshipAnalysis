...
class TestBackoffHandlerShouldRetry:
    ...
    def test_should_retry_max_attempts_reached(self, backoff_handler):
        ...
        result = backoff_handler.should_retry(
            url, status_code, attempt=4
        )  # max_retries = 3
        assert result is False

        # Should not retry when attempt equals max_retries + 1
        result = backoff_handler.should_retry(
            url, status_code, attempt=backoff_handler.max_retries + 1
        )
        assert result is False
...
class TestBackoffHandlerPrivateMethods:
    ...
    def test_get_domain_from_url(self, backoff_handler):
        ...
        assert (
            backoff_handler._get_domain("http://subdomain.example.org/page?param=value")
            == "subdomain.example.org"
        )
        assert (
            backoff_handler._get_domain("https://www.fandom.com/wiki/Page")
            == "www.fandom.com"
        )
    ...
    def test_calculate_delay_exponential(self, backoff_handler):
        ...
        delay2 = backoff_handler._calculate_delay(
            2
        )  # base_delay * 2^1 = base_delay * 2
        delay3 = backoff_handler._calculate_delay(
            3
        )  # base_delay * 2^2 = base_delay * 4
...
class TestBackoffHandlerEdgeCases:
    ...
    def test_should_retry_invalid_status_code(self, backoff_handler):
        ...
        for status_code in invalid_codes:
            ...
            assert (
                result is False
            ), f"Should not retry invalid status code {status_code}"
...
class TestBackoffHandlerConfiguration:
    ...
    def test_very_long_delays(self):
        ...
        backoff_handler = BackoffHandler(
            base_delay=60.0, max_delay=3600.0, max_retries=5
        )
        ...
        assert (
            delay >= 45.0
        )  # Should be close to base_delay (accounting for negative jitter)
